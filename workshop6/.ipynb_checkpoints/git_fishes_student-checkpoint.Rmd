...
datase <- data %>% select(-comment,-loc, -animal_id)
...
datase %>% ggplot(aes(y = activity)) + geom_histogram(aes(fill = treatment), binwidth = 3) + facet_wrap(~species) 
ggsave("./.gitignore/output/figures/stat_all.png")
```
...
for (i in unique(datase$species)){
   ...
   datase[which(datase$species == i),] %>% ggplot(aes(x = activity)) + ylim(0,10) + xlim(0,75) +
   annotation_raster(fish, xmin = 0, xmax = 25, ymin = 8.5, ymax = 10) + geom_histogram(aes(fill = treatment),binwidth = 3)
   ggsave(str_c("./.gitignore/output/figures/stat_",i,".png"))
}
```
...
# Add your code here
stat_sl <- datase %>% group_by(species) %>% 
   summarise(broom::tidy(t.test(sl~treatment, data = cur_data())), .groups = "keep") %>% 
   select(species,statistic,p.value,conf.low,conf.high) %>% rename(t_value = statistic)
stat_activity <- datase %>% group_by(species) %>% 
   summarise(broom::tidy(t.test(activity~treatment, data = cur_data())), .groups = "keep") %>% 
   select(species,statistic,p.value,conf.low,conf.high) %>% rename(t_value = statistic)
```
