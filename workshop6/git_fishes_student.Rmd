...
datase <- data %>% select(-comment, -animal_id)
...
datast <- datase %>% filter(!is.na(species)) %>% filter(!is.na(treatment)) %>% 
   group_by(species,treatment) %>% summarise(across(where(is.numeric),list(mean = mean,sd = sd))) %>% 
   ungroup() %>% pivot_longer(cols = !c(species,treatment),names_to = c("variable","statistic"),
               names_pattern = "(.*)_(.*)",values_to = "value")
# Use flextable to render the summary table in a tidy format
flextable(datast) %>% bold(part = "header") %>% add_footer_lines("The statistic of species-treatment")
