...
for (i in unique(datase$species)){
   ...
   annotation_raster(fish, xmin = 1.8, xmax = 2.5, ymin = 60, ymax = 70) + geom_boxplot(aes(fill = loc)) 
...
path <- "./data/OA_activitydat_20190302_BIOL3207.csv"
data <- read_csv(path)
data %>% filter(species==c("chromis","lemon","acantho"))%>% ggplot(aes(x = treatment, y = activity)) + geom_boxplot(aes(fill = loc)) + facet_wrap(~species) 
```
...
stat_activity <- datase %>% group_by(species) %>% 
   ...
   select(-parameter,-method,-alternative) %>% rename(t_value = statistic)
```
...
Mean activity for `r "acantho"` in the control group was `r stat_activity[[which(stat_activity$species == "acantho"),"estimate"]]` (s / min) compared to the OA treatment group, which was `r stat_activity[[which(stat_activity$species == "acantho"),"estimate1"]]` (s / min). The difference between control and OA treatment means was `r stat_activity[[which(stat_activity$species == "acantho"),"estimate2"]]` (s / min) (95% CI: `r stat_activity[[which(stat_activity$species == "acantho"),"conf.low"]]` to `r stat_activity[[which(stat_activity$species == "acantho"),"conf.high"]]`).
...
stat_result <- full_join(permutation_result, stat_activity)
stat_result
```
