...
Use this Rmarkdown file as your template to complete the tasks outlined within the instructions. There are questions asked in the instructions and here in the RMarkdown file. Answer these as best as you can to document your steps and thinking. There are also a series of coding exercises. Throughout we are also going to learn a bit more about how to format Rmarkdown documents. So, pay close attention to the formatting. We're also going to make use of the `bookdown` package now to render your Rmarkdown because, well, it's quite elegant in many ways. Before you start the tasks install the following packages:

```{r loadpacks, results='hide', message=FALSE}
...
[My GitHub Repository](https://github.com/henflower/DataScience_Exercise/tree/main/workshop6)
...
# The role of GitHub is to centrally host different versions and branches and form the master version. 
# For your own backup data should be stored in personal cloud storage, because GitHub admins can delete your fork.
```
...
# if using echo = FALSE, the output of running code will be hidded.
```
...
datase <- data %>% select(-comment)
# Check spelling in species and treatment but also generate a summary table
datast <- datase %>% filter(!is.na(species)) %>% filter(!is.na(treatment)) %>% 
   group_by(species,treatment) %>% summarise(across(c(sl,activity),list(mean = mean,sd = sd)),size = length(unique(animal_id)), .groups = "drop")
# Use flextable to render the summary table in a tidy format
flextable(datast) %>% bold(part = "header") %>% vline(j = c("treatment","sl_sd","activity_sd")) %>% add_header_row(values = c("group","sl","activity","sample"),colwidth = c(2,2,2,1)) %>% set_header_labels(sl_mean = "mean",sl_sd ="std",activity_mean = "mean",activity_sd = "std") %>% add_header_lines("The statistic of species-treatment")
```
...
You can see the comparison between the local content and the content on Github,
where green represents the added content, and red represents the modified and deleted content.
```
...
```{r prettyfig, fig.align="center", fig.cap="Activity difference"}
# ggplot figure showing mean activity for each treatment (Control, OA) for each species.
datase %>% ggplot(aes(x = treatment, y = activity)) + geom_boxplot(aes(fill = loc)) + facet_wrap(~species) + theme_bw()+ xlab('Species') + ylab('Activity (s)')+ ggtitle("Activity distribution in different treatment for each species")
ggsave("./output/figures/stat_all.png")
```
...
for (i in unique(datase$species)){
   fish = readPNG(str_c("./pics/",i,".png"))
   datase[which(datase$species == i),] %>% ggplot(aes(x = treatment, y = activity))+ ylim(0,70) +
   annotation_raster(fish, xmin = 1.8, xmax = 2.5, ymin = 60, ymax = 70) + geom_boxplot(aes(fill = loc)) + ggtitle(str_c(" Activity of ",i,"under different treatment"))
   ggsave(str_c("./output/figures/stat_",i,".png"))
}
```
...
```{r answerQ6, echo=TRUE, eval=FALSE}
Using output/ will block all folders with the same location name, but files already uploaded to output/ will not be deleted.
```
...
I don't want to keep track of the images in the pic/ folder on Github, because people can generate images locally from the source code, and once the details of the image are modified, it is difficult to merge or modify like the code, and it takes up a lot of space in the repository。
```
...
```{r collabFig, fig.cap="collaborate", echo=TRUE, eval=TRUE}
...
datase %>% filter(species == c("chromis","lemon","acantho")) %>% ggplot(aes(x = treatment, y = activity, fill = treatment)) +
  geom_violin(trim = F) +facet_wrap(~species)+
  geom_boxplot(width = 0.45)+
  geom_jitter(aes(colour = treatment),size = 0.9,width=0.1,height=0) + theme_bw()+ xlab('Species') + ylab('Activity (s)')+
...
ggsave("./output/figures/stat_all.png")
=======
datase %>% ggplot(aes(x = treatment, y = activity)) + geom_boxplot(aes(fill = loc)) + facet_wrap(~species) 
ggsave("./output/figures/stat.png")
>>>>>>> 31da982a7d0fb657443abdab06af3fe853e5abaf
```
...
# Add your code here
stat_activity <- datase %>% group_by(species) %>% 
   summarise(broom::tidy(t.test(activity~treatment, data = cur_data())), .groups = "drop") %>% 
   select(-parameter,-method,-alternative) %>% rename(t_value = statistic, diff_name = estimate, mean1 = estimate1, mean2 = estimate2)
```

```{r table1, echo=TRUE, eval=TRUE, tab.cap="t-test for all species"}
...
knitr::kable(stat_activity,caption = "t-test for all species")
```
...
Mean activity for `r "acantho"` in the control group was `r stat_activity[[which(stat_activity$species == "acantho"),"diff_mean"]]` (s / min) compared to the OA treatment group, which was `r stat_activity[[which(stat_activity$species == "acantho"),"mean1"]]` (s / min). The difference between control and OA treatment means was `r stat_activity[[which(stat_activity$species == "acantho"),"mean2"]]` (s / min) (95% CI: `r stat_activity[[which(stat_activity$species == "acantho"),"conf.low"]]` to `r stat_activity[[which(stat_activity$species == "acantho"),"conf.high"]]`).
...
```{r stretch2, echo=TRUE, eval=TRUE, tags=c()}
# Add your code here.
n_rep <- 1000
permutation_result <- tibble()
for (group in unique(datase$species)){
   samp_dist <- rep(0,n_rep)
   selected_data <- datase[which(datase$species == group),] 
   for (i in 1:n_rep){
      samp_dist[i] <- selected_data %>% mutate(treatment = treatment[sample(row_number())]) %>%
         t.test(activity ~ treatment, data = .) %>% .$statistic
   }
   t_value <- datase[which(datase$species == group),] %>% t.test(activity ~ treatment, data = .,var.equal=T) %>% .$statistic
   permutation_pvalue <- sum(samp_dist>= t_value)/n_rep
   permutation_result <- rbind(permutation_result, tibble(species = group,P_value = permutation_pvalue))
}
colnames(permutation_result) <- c("species","Per.P.value")
stat_result <- full_join(permutation_result, stat_activity, by = "species")
stat_result %>% select(species,Per.P.value,p.value)
```
...
After `r n_rep` permutation tests, the permutation test for the lemon group gave a p-value of `r stat_result[[which(stat_result$species == "lemon"),"Per.P.value"]]` and the t-test gave a p-value of `r stat_result[[which(stat_result$species == "lemon"),"p.value"]]`. The permutation test for the humbug group gave a p-value of `r stat_result[[which(stat_result$species == "humbug"),"Per.P.value"]]`, and the t-test gave p-value -value is also `r stat_result[[which(stat_result$species == "humbug"),"p.value"]]`, and the null hypothesis can be rejected even if a is 0.05. Both p-values for the remaining species were greater than 0.2.
...
[My index.html](https://henflower.github.io/DataScience_Exercise/)
